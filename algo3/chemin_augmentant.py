...
class MaxFlowSolver:
    ...
    def bfs(self, parent):
        visited = [False] * self.n  # mark all nodes as unvisited
        ...
        while queue:
            ...
            for v in range(self.n):
                ...
                if not visited[v] and self.graph[u][v] > 0:
                    ...
                    parent[v] = u  # update the parent of the node
        return visited[self.t]  # return True if there is a path from source to sink

    def ford_fulkerson(self):
        parent = [-1] * self.n  # initialize the parent list with -1
...
if __name__ == '__main__':
    ...
    with open("ford_fulkerson_results.txt", 'w') as f:
        for file, data in res.items():
            f.write("filename : " + file + "\n")
            f.write("max flow : " + str(data[0]) + "\n")
            f.write("time : " + str(data[1]) + "\n")
            f.write("\n")
